...
Borrar su registro del JSON, su página (`ventas/ven_<id>.html` o `alquileres/alq_<id>.html`), su redirección de la raíz si es de las viejas (`ven_<id>.html` o `alq_<id>.html`) y su carpeta de fotos. El generador no borra páginas viejas.
...
- **Links viejos (QR, WhatsApp):** antes las páginas de cada propiedad estaban en la raíz (`/ven_x.html`). Para las propiedades que ya existían en ese momento (lista `REDIRECCIONES_VIEJAS` en `_build/build.py`) el generador deja en la raíz una pequeña página que redirige a la nueva dirección (`/ventas/ven_x.html`), así los QR impresos y los links ya compartidos siguen funcionando. Las propiedades nuevas no generan estos archivos. Cuando ya no queden QRs viejos, se puede vaciar esa lista y borrar esos archivos de la raíz.
